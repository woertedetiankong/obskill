...
- Preserve the helper's sibling `.assets` directories. Do not relocate them into the general attachment folder. When the user requests readable conversation filenames, follow the paired note/asset rename procedure in `history.md`; ordinary saves do not silently rename older exports.
